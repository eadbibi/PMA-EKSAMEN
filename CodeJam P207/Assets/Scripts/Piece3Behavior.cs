...
public class Piece3Behavior : MonoBehaviour
{
    [SerializeField] private Transform piece3Place;
        ...
        if (Input.touchCount > 0 && !locked)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Moved:
                    if (Mathf.Abs(transform.position.x - piece3Place.position.x) <= 0.5f && Mathf.Abs(transform.position.y - piece3Place.position.y) <= 0.5f)
                    {
