...
public class Piece1Behavior : MonoBehaviour
{

    [SerializeField] private Transform piece1Place;
        ...
        if (Input.touchCount > 0 && !locked)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Moved:
                    if (Mathf.Abs(transform.position.x - piece1Place.position.x) <= 0.5f && Mathf.Abs(transform.position.y - piece1Place.position.y) <= 0.5f)
                    {
