...
public class Piece4Behavior : MonoBehaviour
{[SerializeField] private Transform piece4Place;
        ...
        if (Input.touchCount > 0 && !locked)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Moved:
                    if (Mathf.Abs(transform.position.x - piece4Place.position.x) <= 0.5f && Mathf.Abs(transform.position.y - piece4Place.position.y) <= 0.5f)
                    {
